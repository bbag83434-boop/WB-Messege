...
export default function AuthScreen({ onAuthenticated }) {
  ...
  return <main className="auth-page"><motion.section className="auth-card" initial={{ opacity: 0, y: 22, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: .45 }}>
    ...
    {step === 'otp' && <form onSubmit={verify} noValidate><div className="otp-inputs" aria-label="Six digit OTP">{otp.map((value, index) => <input key={index} ref={(element) => { inputs.current[index] = element }} value={value} onChange={(event) => changeOtp(index, event.target.value)} onKeyDown={(event) => { if (event.key === 'Backspace' && !otp[index] && index) inputs.current[index - 1]?.focus() }} inputMode="numeric" autoComplete={index === 0 ? 'one-time-code' : 'off'} maxLength="6" aria-label={`OTP digit ${index + 1}`} />)}</div>{developmentOtp && <p className="auth-dev-otp" style={{ cursor: 'pointer' }} onClick={() => setOtp(developmentOtp.split(''))} title="Click to auto-fill OTP">Personal mode OTP: <strong>{developmentOtp}</strong> (Click to auto-fill)</p>}<Status error={error} success={success} /><button className="auth-submit" disabled={loading}>{loading ? 'Verifying…' : 'Verify OTP'}</button><p className="auth-switch"><button type="button" onClick={startOver}>Change mobile number</button>{seconds ? <span> · Resend in {seconds}s</span> : <button type="button" onClick={send} disabled={loading}> · Resend OTP</button>}</p></form>}
