...
function getDeviceId(req) { return req.body?.deviceId || req.get('x-device-id') || null }
...
function createToken(user, expiresIn = '30d', purpose) { return jwt.sign({ jti: randomUUID(), sub: user.id, mobile: user.mobile, ...(purpose ? { purpose } : {}) }, process.env.JWT_SECRET, { expiresIn }) }
...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    req.auth = decoded
    if (decoded.jti) {
      const revoked = await database.query('SELECT 1 FROM revoked_tokens WHERE token_id = $1', [decoded.jti])
      if (revoked.rowCount > 0) return res.status(401).json({ message: 'Your session has been terminated. Please sign in again.' })
    }
    return next()
...
app.post('/api/auth/send-otp', async (req, res, next) => {
  try {
    ...
    // Check if account already exists and bound to another device
    const existingUserRes = await database.query('SELECT device_id FROM users WHERE mobile = $1', [mobile])
    if (existingUserRes.rowCount > 0 && existingUserRes.rows[0].device_id && deviceId && existingUserRes.rows[0].device_id !== deviceId) {
      return res.status(403).json({ message: 'This mobile number is linked to another device.' })
    }
    ...
    await database.query('INSERT INTO otp_codes (id, mobile, code_hash, expires_at, device_id) VALUES ($1, $2, $3, NOW() + make_interval(mins => $4::integer), $5)', [randomUUID(), mobile, await bcrypt.hash(otp, 12), OTP_TTL_MINUTES, deviceId])
...
app.post('/api/auth/verify-otp', async (req, res, next) => {
  try {
    ...
    const result = await database.query('SELECT id, code_hash AS "codeHash", expires_at AS "expiresAt", attempt_count AS "attemptCount", used_at AS "usedAt", device_id AS "deviceId" FROM otp_codes WHERE mobile = $1 ORDER BY created_at DESC LIMIT 1', [mobile])
    const record = result.rows[0]
    if (!record) return res.status(401).json({ message: 'That OTP is invalid or has expired.' })

    if (record.deviceId && deviceId && record.deviceId !== deviceId) {
      return res.status(403).json({ message: 'This OTP was requested from another device.' })
    }

    if (record.attemptCount >= 5) {
      return res.status(401).json({ message: 'Too many failed attempts. This OTP has been invalidated.' })
    }

    if (record.usedAt) {
      return res.status(401).json({ message: 'That OTP is invalid or has expired.' })
    }
    ...
    if (!isValid) {
      ...
      if (newAttemptCount >= 5) {
        await database.query('UPDATE otp_codes SET attempt_count = $1, used_at = NOW() WHERE id = $2', [newAttemptCount, record.id])
        return res.status(401).json({ message: 'Too many failed attempts. This OTP has been invalidated.' })
      } else {
        await database.query('UPDATE otp_codes SET attempt_count = $1 WHERE id = $2', [newAttemptCount, record.id])
        return res.status(401).json({ message: 'That OTP is invalid or has expired.' })
      }
    }

    // Mark OTP as single-use (used_at = NOW())
    await database.query('UPDATE otp_codes SET used_at = NOW() WHERE id = $1', [record.id])
    
    const userResult = await database.query('SELECT id, name, mobile, device_id AS "deviceId", profile_photo AS "profilePhoto", about, created_at AS "createdAt", last_seen AS "lastSeen", is_online AS "isOnline" FROM users WHERE mobile = $1', [mobile])
    const user = userResult.rows[0]
    if (!user) return res.json({ registrationRequired: true, verificationToken: createToken({ id: `verify:${mobile}`, mobile, ...(deviceId ? { deviceId } : {}) }, '10m', 'mobile-verification') })
    
    // Check device binding if existing user has a bound device
    if (user.deviceId && deviceId && user.deviceId !== deviceId) {
      return res.status(403).json({ message: 'This mobile number is linked to another device.' })
    }

    if (!user.deviceId && deviceId) {
      await database.query('UPDATE users SET device_id = $1 WHERE id = $2', [deviceId, user.id])
    }
...
app.post('/api/auth/register', requireVerifiedMobile, async (req, res, next) => {
  try {
    ...
    const deviceId = getDeviceId(req) || req.verification.deviceId || null
    ...
    const result = await database.query('INSERT INTO users (id, name, mobile, profile_photo, device_id, is_online, last_seen) VALUES ($1, $2, $3, $4, $5, true, NOW()) ON CONFLICT (mobile) DO NOTHING RETURNING id, name, mobile, profile_photo AS "profilePhoto", about, created_at AS "createdAt", last_seen AS "lastSeen", is_online AS "isOnline"', [randomUUID(), name, mobile, suppliedPhoto || avatarFor(name), deviceId])
...
app.post('/api/auth/logout', requireAuth, async (req, res, next) => {
  try {
    ...
    if (tokenId) {
      ...
      await database.query('INSERT INTO revoked_tokens (token_id, user_id, expires_at) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING', [tokenId, userId, expiresAt])
    }
    await database.query('UPDATE users SET device_id = NULL, is_online = false, last_seen = NOW() WHERE id = $1', [userId])
    return res.status(204).end()
  } catch (error) { return next(error) }
})
